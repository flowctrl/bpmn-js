...
PaletteProvider.prototype.getPaletteEntries = function(element) {
  ...
  if (window.pal.consumer != 'hwenc') {
    options['create.intermediate-event'] = createAction('bpmn:IntermediateThrowEvent', 'event',
        'bpmn-icon-intermediate-event-none', translate('Create IntermediateThrowEvent/BoundaryEvent')
    );
  }

  options['create.end-event'] = createAction(
      'bpmn:EndEvent', 'event', 'bpmn-icon-end-event-none'
  );

  options['create.exclusive-gateway'] = createAction(
      'bpmn:ExclusiveGateway', 'gateway', 'bpmn-icon-gateway-xor'
  );

  options['create.task'] = createAction(
      'bpmn:Task', 'activity', 'bpmn-icon-task'
  );

  if (window.pal.consumer == 'hwenc') {
    options['create.call-activity'] = createAction(
        'bpmn:CallActivity', 'activity', 'bpmn-icon-call-activity'
    );
  }

  if (window.pal.consumer != 'hwenc') {
    options['create.data-object'] = createAction(
        'bpmn:DataObjectReference', 'data-object', 'bpmn-icon-data-object'
    );
  }

  options['create.data-store'] = createAction(
      'bpmn:DataStoreReference', 'data-store', (window.pal.consumer == 'hwenc') ? 'bpmn-icon-data-store-single' : 'bpmn-icon-data-store'
  );

  if (window.pal.consumer != 'hwenc') {
    options['create.subprocess-expanded'] = createAction(
        'bpmn:SubProcess', 'activity', 'bpmn-icon-subprocess-expanded', translate('Create expanded SubProcess'),
        { isExpanded: true }
    );

    options['create.participant-expanded'] = {
      group: 'collaboration',
